document.addEventListener("DOMContentLoaded", async () => {
  ...
  setupPinBtn.addEventListener("click", async () => {
    ...
    if (newPin && newPin.length === 4 && !isNaN(newPin)) {
      ...
      alert(
        "PIN set successfully! Sensitive actions will now require this PIN."
      );
    } else if (newPin !== null) {
  ...
  function toggleKeywordSection(show) {
    if (show) {
      ...
      // Small timeout to allow display change before opacity transition if needed
      setTimeout(() => {
        keywordSection.style.opacity = "1";
      }, 10);
    } else {
      keywordSection.classList.add("hidden");
      keywordSection.style.opacity = "0";
    }
  }
